...
// Definisikan tipe untuk prop `pertanyaan`
...
export default function EditPertanyaan({ pertanyaan }: { pertanyaan: Pertanyaan }) {
  const { data, setData, put, processing, errors } = useForm({
    teksPertanyaan: pertanyaan.teksPertanyaan,
    tipeRiasec: pertanyaan.tipeRiasec,
  })

  function submit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault()
    // Gunakan method 'put' dan URL yang sesuai
    put(`/admin/pertanyaan/${pertanyaan.id}`)
  }

  return (
    <>
      <Head title={`Edit Pertanyaan - #${pertanyaan.id}`} />
      <div className="p-8 max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">
            Edit Pertanyaan RIASEC
          </h1>
          <Link
...
EditPertanyaan.layout = (page: any) => <AdminLayout children={page} />
